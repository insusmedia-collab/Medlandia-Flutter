...
import 'package:medlandia/screens/visitCard.dart';
...
class MessageScreen extends StatefulWidget {  
  static MessageQuee openedQuee = MessageQuee(messageUniqId: -1, type: MQtypes.NORMAL);
...
class _MessageScreenState extends State<MessageScreen> {
            ...
            IconButton(
              onPressed: () {
                Navigator.push(
                  ...
                  MaterialPageRoute(
                    builder: (context) { 
                      List<Recipient> users = List.from(MessageScreen.openedQuee.getUsers());
                      return  MessageUserChooser(list: users);
                      },
                  ),
                ).then((value) {
                    MessageScreen.openedQuee.replaceAllUsers(value);
                    MessageScreen.openedQuee.userListChanged.value = !MessageScreen.openedQuee.userListChanged.value;
                });
              },
                      ...
                      child: ValueListenableBuilder(
                        ...
                        builder:
                            (a, b, c) =>  ListView.builder(
                              ...
                              itemBuilder:
                                  (context, index) => buildUserArea(MessageScreen.openedQuee.messages[index] as BaseMessageModel),
                            ),
                      ),                    
                    ),
                    if (attachedMemberLinked.length > 0) Divider(height: 5),
                    Align(
                      ...
                      child: ConstrainedBox(
                        ...
                        child: SingleChildScrollView(
                          child: ValueListenableBuilder(
                            valueListenable: attachedLinkedMemberChanged,
                            builder: (_, __, ___) => buildLinkedMembersChoosedWidget(),
                          ),
                        ),
                      ),
                    ),
                      ...
                      child: ConstrainedBox(
                        ...
                        child: SingleChildScrollView(
                          child: ValueListenableBuilder(
                            ...
                            builder: (_, __, ___) => buildFilesChoosedWidget(),
                          ),
                        ),
                      ),
                    ),
                      ...
                      child: ConstrainedBox(
                        ...
                        child: SingleChildScrollView(
                          child: ValueListenableBuilder(
                            ...
                            builder: (_, __, ___) => buildImagesChoosedWidget(),
                          ),
                        ),
                      ),
                    ),
                ...
                if (value == MsgStatus.SENDING) {
                  ...
                  return Column(
                    ...
                    children: [
                      ...
                      getImages(m),
                      getFiles(m),
                      for (Recipient ml in m.linkedMembers) 
                        VisitCart(r: ml),
                      ValueListenableBuilder(
        ...
        Row(
          children: [
            ...
            PopupMenuButton<String>(
              onSelected: (value) {
                if (value == "linkusers") {
                  Navigator.push(context, MaterialPageRoute(builder: (context) => MessageUserChooser(list: attachedMemberLinked)))
                  .then((value) {                    
                    attachedMemberLinked = value;
                    attachedLinkedMemberChanged.value = !attachedLinkedMemberChanged.value;
                  });
                }
              },
  ...
  Widget buildLinkedMembersChoosedWidget() {
     if (attachedMemberLinked.length == 0) {
      return SizedBox();
    }
    return GridView.builder(
      padding: const EdgeInsets.all(8),
      itemCount: attachedMemberLinked.length,
      shrinkWrap: true,
      physics: const NeverScrollableScrollPhysics(),
      gridDelegate: const SliverGridDelegateWithMaxCrossAxisExtent(
          ...
          children: [
            ClipRRect(
              ...
              child: Column(
                children: [
                  CircleAvatar(
                    radius: 30,
                    backgroundImage: NetworkImage("https://medlandia.org/medlandia.jsp?func=getAvatar&p1=${attachedMemberLinked[index].id}"),
                    backgroundColor: Colors.grey.shade200,
                  ),                  
                  Text(
                    attachedMemberLinked[index].name,
            ...
            Positioned(
              ...
              child: GestureDetector(
                onTap: () {
                  attachedMemberLinked.removeAt(index);
                  attachedLinkedMemberChanged.value = !attachedLinkedMemberChanged.value;
                },
                child: CircleAvatar(
                  radius: 12,
  ...
  Widget buildFilesChoosedWidget() {
  ...
  Widget buildImagesChoosedWidget() {
  ...
  Future<BaseMessageModel?> toMessage(String? subject, String? text, List<File> files, List<XFile> images) async {
    ...
    if (_controller.text.trim().length == 0 &&
        ...
        attachedImages.length == 0 && attachedMemberLinked.length == 0) {
    ...
    for (Recipient r in attachedMemberLinked) {
      m.addLinkedMember(r);
    }
    return m;
  }
