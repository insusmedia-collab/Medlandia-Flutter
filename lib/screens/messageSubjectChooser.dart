...
class MessageUserChooser extends StatefulWidget {
  ...
  const MessageUserChooser({super.key, required this.list});
      ...
      body: SafeArea(
        child: ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            for (Recipient r in widget.list) {
                ...
                setState(() {
                    if (isSelected) {
                      ...
                      for (int i = widget.list.length-1; i >= 0; i--) {
                        if (widget.list[i].id == _filteredItems[index].id) {
                          widget.list.removeAt(i);
                        }
                      }
                    } else {
                      widget.list.add( Recipient(id: _filteredItems[index].id, name: _filteredItems[index].name) );
                    }
                  });
              },
            );
