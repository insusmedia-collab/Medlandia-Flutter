...
MessageQuee wrapMessageQuee(Map<String, dynamic> row) {
  ...
  MQtypes type = MQtypes.NORMAL;

  if (msgs.contains("type='headline'") || msgs.contains('type="headline"')) { // headline messages cant conain normal message values
    type = MQtypes.SYSTEM;
  }

  MessageQuee q = MessageQuee(messageUniqId: row['id'], type: type);
    ...
    } else if (type == MQtypes.SYSTEM) {
      for (var r in msgChildren) {
        HeadlineMessage? m = HeadlineMessage.fromXML(r);
        if (m == null) {
          print("--Error-- parsing message from database ${r.toXmlString()}");
          // BROCKEN MESSAGE -> FUTURE DELETE IT
          continue;
        }
        q.messages.add(m);
      } 
...
Future<int> db_getAllMesagesCount() async {
  final db = await Dbase.instance.database;
