...
import 'package:medlandia/screens/systemCleaner.dart';
...
class MainApp extends StatelessWidget  /*with WidgetsBindingObserver */ {
  ...
  Future<String> getStartWidget() async {    
    ...
    await loadContent();
    lastDataBaseCleanStr = await LocalStore.read("databaseCleanDate");
    //await dropDatabase();
    //await dump();
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            if (currentUser == null) {
              ...
              bool isUpdatable = (version != null &&  version['version'] != null && version['version'].toString().trim() != appVersion!.trim() 
                  || appVersion != null && version['buildNumber'] != null && version['buildNumber'].toString().trim() != appBuildNumber!.trim());
              ...
              if (isUpdatable) {
                ...
                if (lastDataBaseCleanStr == null) {
                  ...
                  DateTime lastCleanDate = DateTime.parse(lastDataBaseCleanStr!);  
                  if (DateTime.now().difference(lastCleanDate).inDays >= 1) {
